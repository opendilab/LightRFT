...
> **实现状态说明：** 当前源码中，GRPO、CPGD、FIRE Sampling、DAPO 的部分机制以及高熵 token 筛选具有明确执行路径。GSPO 尚未接入实际实例化的策略损失，REINFORCE++ 尚未映射到优势计算器工厂，GMPO/Dr.GRPO 也不是完整的端到端选项。下文相关小节仅作为设计说明保留，其中的示例命令不构成 0.1.1 版本的可运行接口；已确认的缺口见 `docs/TODO.md`。

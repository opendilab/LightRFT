...
T2I_TRAINING_DATA_PATH="path/to/t2i_rejection_sampling_train.json"

# 视频拒绝采样数据（convert_to_rejection_sampling_data_t2v.py 的输出）
T2V_TRAINING_DATA_PATH="path/to/t2v_rejection_sampling_train.json"

# 输出目录
OUTPUT_DIR="./results/rejection_sampling_mix_$(date +%Y%m%d_%H%M%S)/checkpoint"
LOG_DIR="$(dirname "${OUTPUT_DIR}")/logs"

# 训练超参（与 examples/grm_training/run_grm_vl.sh 对齐）
TBS=2                       # global train batch size（与 run_grm_vl.sh 中 TBS 一致）
LR=1e-5                     # 与 run_grm_vl.sh 中 LR 一致
MAX_LENGTH=8196             # 与 run_grm_vl.sh 中 MAX_LENGTH 一致
MAX_EPOCHS=3                # 与 run_grm_vl.sh 中 --max_epochs 5 一致
MICRO_BATCH_SIZE=1          # 与 run_grm_vl.sh 中 micro_train_batch_size 4 一致
GRADIENT_ACCUMULATION_STEPS=1  # 当前脚本未显式传入该参数，仅作记录

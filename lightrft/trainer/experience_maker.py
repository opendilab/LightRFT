...
class RemoteExperienceMaker(NaiveExperienceMaker):
    """
    Remote experience maker using vLLM engines for distributed generation.

    This class extends NaiveExperienceMaker to support distributed generation using
    vLLM engines via Ray, enabling efficient large-scale rollout generation.

    :param args: Positional arguments passed to NaiveExperienceMaker.
    :type args: tuple
    :param vllm_engines: List of vLLM engine actors for distributed generation, defaults to None.
    :type vllm_engines: List, optional
    :param packing_samples: Whether to use packed sample format, defaults to False.
    :type packing_samples: bool
    :param kwargs: Keyword arguments passed to NaiveExperienceMaker.
    :type kwargs: dict
    """
    ...
    def make_experience_list(self, all_prompts: Union[str, List[str]], **generate_kwargs) -> List[Experience]:
        """
        Make a list of experiences with performance tracking.
    ...
    def generate_samples(self, all_prompts: List[str], **generate_kwargs) -> List[Samples]:
        ...
        When not using vLLM, we fallback to the default implementation,
    ...
    def make_experience(self, samples: Samples) -> Experience:
        ...
        This method uses remote actors for initial model, critic, and reward model to enable
        distributed computation and model colocation.
        ...
        # Initial model log probabilities (remote)
        if self.initial_model is not None:
        ...
        # Critic values (remote)
        if self.critic is not None:
            value_ref = self.critic.forward.remote(
                sequences_cpu, num_actions, attention_mask_cpu, packed_seq_lens=packed_seq_lens
            )
            # Avoid CUDA OOM when colocating models
        ...
        if not self.remote_rm_url:
            ...
            if self.custom_reward_func:
                ...
                for rm in self.remote_rm_url:
                    r = remote_rm_fn.remote(rm, queries=queries, prompts=samples.prompts)
                    r_refs.append(r)
        ...
        # Wait for initial/critic/reward model to complete
        start = time.time()
        ...
        # Reward function returns (reward, metrics_dict), need to unpack both
        if len(rewards) > 0:
            r, reward_metrics = self.reward_fn(rewards)
        else:
            r = rewards[0]
            reward_metrics = None
        ...
        # Add reward_metrics if available (contains format_reward, accuracy_reward, model_reward, rule_reward)
        if 'reward_metrics' in locals() and reward_metrics is not None:
    ...
    def _generate_vllm(self, all_prompts: List[str], **kwargs) -> List[Samples]:
        """
        Generate samples using vLLM engines.

        :param all_prompts: List of prompt strings.
        :type all_prompts: List[str]
        :param kwargs: Generation parameters (temperature, top_p, max_tokens, etc.).
        :type kwargs: dict
        :return: List of Samples objects.
        :rtype: List[Samples]
        """
        ...
        for i in range(0, len(all_outputs), args.micro_rollout_batch_size):
            ...
            if not self.packing_samples:
                ...
                # NOTE: Concat all outputs to following format:
                #
                # | token token token | token token [EOS] | token token token token token | token token [EOS] | token token | token token token [EOS] |  # noqa
                # |<---  prompt ----->|<---- answer ----->|<---------- prompt ----------->|<----- answer ---->|<- prompt -->|<-------- answer ------->|  # noqa
                pad_token_id, eos_token_id = self.tokenizer.pad_token_id, self.tokenizer.eos_token_id
                sequences = []
                packed_seq_lens = []
                attention_mask = []
    ...
    def flush(self):
        """
        Ensure all experiences have been sent to critic.
        """
        if self.critic is not None:
            ray.get(self._ref)
            self._ref = None

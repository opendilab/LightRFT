...
class TrajectorySaver:
    ...
    def _unpack_experience_to_dicts(self, exp: Any, step: int, exp_idx: int) -> List[Dict[str, Any]]:
        ...
        # Extract labels and references from Experience object
        # Labels: e.g., "gsm8k_rule" for recipe-based reward
        # References: e.g., ground truth answers like "42" for GSM8K problems
        labels = (
            exp.labels
            if hasattr(exp, 'labels') and exp.labels is not None
            else [None] * batch_size
        )
        references = (
            exp.references
            if hasattr(exp, 'references') and exp.references is not None
            else [None] * batch_size
        )

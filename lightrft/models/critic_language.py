"""
Language Critic Model Module for Reinforcement Learning.

This module provides the CriticLanguage class, which implements a critic model specifically designed
for pure language tasks in reinforcement learning scenarios. The critic is responsible for
estimating state values based on textual prompts.
...
- Support for various language model architectures
...
class CriticLanguage(nn.Module):
    """
    Language Critic model for reinforcement learning applications.

    This class implements a critic model for PPO/GRPO training that estimates state values
    for advantage calculation. It extends a base language model with a linear value
    ...
    Example::
        ...
        critic = CriticLanguage(
            pretrain_or_model="meta-llama/Llama-2-7b-hf",
